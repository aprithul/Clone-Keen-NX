...
char Load_Episode(uchar episode)
{
		...
		strcmp(episode_loaded_from_path, levelcontrol.custom_episode_path))
	{
		...
		lprintf("Loaded special sprites");
		
		if (loadtileattributes(episode)) return 1;
		lprintf("Loaded tile attributes");
		
		if (initgplsprites(episode)) return 1;
		lprintf("init gpl  sprites");
...
int i;
	...
	if (!fpo) { crash("save_compress_demo: Unable to open output file"); fclose(fpi); return; }
	
	fputc('D', fpo); fputc('M', fpo); fputc('O', fpo);
	fputc(DEMOVERSION, fpo);
	fputc(levelcontrol.episode, fpo);
	fputc(levelcontrol.curlevel, fpo);
	...
	do
	{
		l = fgetc(fpi); r = fgetc(fpi);
		j = fgetc(fpi); p = fgetc(fpi);
		f = fgetc(fpi); stop = fgetc(fpi);
		if (feof(fpi) && !stop)
		{
