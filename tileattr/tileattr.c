...
int t;
		...
		strcat(fname, filename);
          ...
          fputc('R', fp);
          fputc(episode, fp);
          fputc(WRITES_ATTRFILEVERSION, fp);

          for(t=0;t<MAX_TILES-1;t++)
          {
            fputc(tiles[t].solidl, fp);
            fputc(tiles[t].solidr, fp);
            fputc(tiles[t].solidfall, fp);
            fputc(tiles[t].solidceil, fp);
            fputc(tiles[t].ice, fp);
            fputc(tiles[t].semiice, fp);
            fputc(tiles[t].priority, fp);
            fputc(tiles[t].masktile, fp);
            fputc(tiles[t].goodie, fp);
            fputc(tiles[t].standgoodie, fp);
            fputc(tiles[t].pickupable, fp);
			fputi(tiles[t].points, fp);
            fputc(tiles[t].lethal, fp);
            fputc(tiles[t].bonklethal, fp);
			fputi(tiles[t].chgtile, fp);
            fputc(tiles[t].isAnimated, fp);
            fputc(tiles[t].animOffset, fp);
            fputc(tiles[t].animlength, fp);
          }
          fclose(fp);
}
