...
unsigned int msb, lsb;
    ...
    {
       fputc(player[0].keytable[PKLEFT], demo.fp);
       fputc(player[0].keytable[PKRIGHT], demo.fp);
       fputc(player[0].keytable[PKJUMP], demo.fp);
       fputc(player[0].keytable[PKPOGO], demo.fp);
       fputc(player[0].keytable[PKFIRE], demo.fp);
       fputc(keytable[KF1], demo.fp);
	   if (keytable[KF1])
	   {
			fclose(demo.fp); demo.fp = NULL;
			save_compress_demo();
